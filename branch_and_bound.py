...
class bnb:
    ...
    def lower_bound_edge_deletion(self, H):
        ...
        """
        solver = approximation.approximation()
    ...
    def still_is_cover(self, node, sub_V, G):
        ...
        if neighbors and not neighbors <= set(sub_V):
            is_cover = False
        return is_cover
    ...
    def bnb_edge_delection(
        ...
        while not sub_problems.isEmpty():
            ...
            if available_vertices_delete_v and self.still_is_cover(
                    max_degree_v, available_vertices_delete_v, remaining_graph,
            ):
                if current_lb < current_best[0]:
                    sub_problems.push(
                        (
...
def run_bnb(input_file, cutoff_time):
    ...
    output_trace = (
        './output/' + filename + '_BnB_' + str(cutoff_time) + '.trace'
    )
    ...
    G = graph_io.read_graph(input_file)
